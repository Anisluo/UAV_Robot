...
// 提取 "params" 字段对应的 JSON 对象原始字符串（含花括号）
...
bool CtrlServer::start(const std::string &path, Handler handler) {
    ...
    if (listen_fd_ < 0) {
        std::fprintf(stderr, "proc_realsense: ctrl socket failed: %s\n", std::strerror(errno));
        return false;
    }
    ...
    if (bind(listen_fd_, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) != 0) {
        std::fprintf(stderr, "proc_realsense: ctrl bind failed: %s\n", std::strerror(errno));
        stop();
        return false;
    }
    if (listen(listen_fd_, 4) != 0) {
        std::fprintf(stderr, "proc_realsense: ctrl listen failed: %s\n", std::strerror(errno));
        stop();
        return false;
    }

...
int RealsenseApplication::run() {
    ...
    // RSCapture::start() falls back to color-only when the depth pipe
    // refuses to come up — that's fine for headless/no-arm setups but
    // it's a silent degradation here: with depth_scale=0 the tracker
    // publishes has_xyz=0 and HostGUI never shows Z=...mm. Unless the
    // operator explicitly opted out (UAV_REALSENSE_DISABLE_DEPTH=1),
    // exit non-zero so systemd's RestartSec=2 retries the whole
    // process — usually a USB / driver race that clears on retry.
    {
        const char *raw = std::getenv("UAV_REALSENSE_DISABLE_DEPTH");
        const bool depth_opt_out =
            (raw != nullptr && raw[0] != '\0' && raw[0] != '0');
        if (!depth_opt_out && !capture.depth_enabled()) {
            std::fprintf(stderr,
                         "proc_realsense: depth pipe failed to start; "
                         "exiting so systemd retries (set "
                         "UAV_REALSENSE_DISABLE_DEPTH=1 to allow "
                         "color-only)\n");

...
bool json_get_method(const std::string &json, std::string &out) {
...
bool CtrlServer::start(const std::string &path, Handler handler) {
    ...
    if (listen_fd_ < 0) {
        std::fprintf(stderr, "proc_grasp: ctrl socket failed: %s\n", std::strerror(errno));
        return false;
    }
    ...
    if (bind(listen_fd_, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) != 0) {
        std::fprintf(stderr, "proc_grasp: ctrl bind failed: %s\n", std::strerror(errno));
        stop();
        return false;
    }
    if (listen(listen_fd_, 4) != 0) {
        std::fprintf(stderr, "proc_grasp: ctrl listen failed: %s\n", std::strerror(errno));
        stop();
        return false;
    }
...
void CtrlServer::handle_client(Client &c) {
    ...
    while ((nl = c.buf.find('\n', start)) != std::string::npos) {
        ...
        (void)json_get_method(line, method);
